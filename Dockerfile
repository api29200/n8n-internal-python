...
# Krok 2: Instalujemy Pythona i podstawowe narzędzia
RUN apk update && apk add --no-cache python3 py3-pip py3-virtualenv

# Krok 3: Inteligentne szukanie folderu runnera i tworzenie .venv
# Przeszukujemy całą strukturę n8n w poszukiwaniu paczki task-runner-python
RUN export RUNNER_DIR=$(find /usr/local/lib/node_modules/n8n -type d -name "*task-runner-python*" | head -n 1) && \
    if [ -z "$RUNNER_DIR" ]; then echo "BŁĄD: Nie znaleziono katalogu task-runner-python!"; exit 1; fi && \
...
# Krok 4: Zapewnienie uprawnień dla katalogu roboczego n8n
